...
from key import key_fallBot, key_tBot, server_id, test_channel, test_id
...
chat_log_channel = (
    1087449487376666624 if current_key == key_fallBot else 1042213192383877263
)
...
class SelectionView(discord.ui.View):
    ...
    async def sel_cluster(self, interaction: discord.Interaction):
        ...
        await interaction.edit_original_response(
            view=self, content=f"Changed cluster to {cluster_name} on {branch} Branch."
        )
...
class PanelMenu(discord.ui.View):
    ...
    async def start_bash(
        ...
        process = subprocess.Popen(
            [start_server_path, cluster_name, str(is_beta_server)]
        )
        msg = await interaction.followup.send(
            "Server startup initated...", ephemeral=True
        )
        if is_beta_server:
            beta_game_version = fs.get_latest_update_info_from_dict(beta=True)
            await client.change_presence(
                activity=discord.Game(name="Beta Don't Starve Together")
            )
        else:
            game_version = fs.get_latest_update_info_from_dict(beta=False)
            await client.change_presence(
                activity=discord.Game(name="Don't Starve Together")
            )
    ...
    async def stop_bash(
        ...
        msg = await interaction.followup.send(
            "Server shutdown initiated...", ephemeral=True
        )
    ...
    async def restart_bash(
        ...
        global is_beta_server, game_version, beta_game_version, cluster_name
        process = subprocess.Popen(
            [restart_server_path, cluster_name, str(is_beta_server)]
        )
        msg = await interaction.followup.send(
            "Server restart initiated...", ephemeral=True
        )
        if is_beta_server:
            beta_game_version = fs.get_latest_update_info_from_dict(beta=True)
        else:
    ...
    async def check_updates(
        ...
        if hf.check_for_updates(is_beta_server, game_version, beta_game_version):
            await interaction.followup.send(
                "There are updates available!", ephemeral=True
            )
        else:
    ...
    async def change_branch(
        ...
        await interaction.response.send_message(
            content=text, view=SelectionView(), ephemeral=True
        )
...
async def new_world(
    interaction: discord.Interaction, cluster_name: str, branch: str, difficulty: str
):
    """
    creates a new world.
    ...
    print(
        str(interaction.user)
        + " attempted to create a new world with name "
        + cluster_name
        + " on branch "
        + branch
        + " with difficulty "
    ...
    if branch != "main" and branch != "beta":
        await interaction.response.send_message(
            "ERROR: Invalid branch.", ephemeral=True
        )
        return
    if difficulty != "main" and difficulty != "relaxed":
        await interaction.response.send_message(
            "ERROR: Invalid difficulty.", ephemeral=True
        )
        return
    ...
    home_dir = os.path.expanduser("~")
    path = (
        os.path.join(home_dir, ".klei", "DoNotStarveTogether", cluster_name)
        if branch == "main"
        else os.path.join(
            home_dir, ".klei", "DoNotStarveTogetherBetaBranch", cluster_name
        )
    )

    if os.path.exists(path):
        await interaction.response.send_message(
            "ERROR: Cluster with the given name already exists.", ephemeral=True
        )
        return

    await interaction.followup.send(
        "Please send a zip file with save files for the world.", ephemeral=True
    )
    ...
    await interaction.followup.send(
        f"Created a new world with name {cluster_name}", ephemeral=True
    )
    await asyncio.sleep(5)
    await interaction.delete_original_response()
...
async def get_player_list(interaction: discord.Interaction):
    ...
    hf.dst_player_list()
    await interaction.response.send_message(
        "Sent Player list to chat log.", ephemeral=True
    )
    await asyncio.sleep(5)
    await interaction.delete_original_response()
...
async def backup(interaction: discord.Interaction, cluster: str, branch: str):
    ...
    await interaction.response.send_message(
        f"Backed up {branch}/{cluster}.", ephemeral=True
    )
    await asyncio.sleep(5)
    await interaction.delete_original_response()
...
async def enable_mod(interaction: discord.Interaction, mod_id: str):
    ...
    if not hf.has_mod_config(cluster_name, is_beta_server, mod_id):
        await interaction.followup.send(
            f"Downloading mod: {mod_id}. This may take a while...", ephemeral=True
        )
